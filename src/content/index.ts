import {
  addCustomBtn,
  addVideoDownloadCoverBtn,
  CLASS_CUSTOM_BUTTON,
  handleVideoCoverDownloadBtn,
  onClickHandler,
} from "./button";
import { initStorageCache, storageCache } from "./extractors/storage";
import { checkType } from "./extractors/fn";
import {
  handleReelsVideoVolumeChange,
  handleStoriesVideoVolumeChange,
  handleVideo,
 ...
 * `src/content/index.ts`. Walks the Instagram/Threads DOM every 3s during
 * idle time and injects the download button family near the like icon on
 * feed posts, reels, stories, highlights, profile avatars, profile-grid
 * video covers, and Threads posts. Global click delegator fires
 * `onClickHandler` for any click on an injected button.
 */

const VIDEO_SVG_PATH =
...
function processPage(): void {
  ...
  const isDark =
    cs === "dark" || window.matchMedia("(prefers-color-scheme: dark)").matches;
  const iconColor: "white" | "black" = isDark ? "white" : "black";
  ...
  if (pathname.startsWith("/p/") || isPostDetailWithNameInUrl || isReelDetailWithNameInUrl) {
    ...
    const btnsContainer =
      ...
      (likeBtn?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement
  ...
  if (pathname.startsWith("/stories/")) {
    ...
    if (node instanceof HTMLDivElement) node.style.pointerEvents = "none";
    const wrapperDiv = Array.from(
      document.querySelectorAll("body>div:not(#splash-screen)>div>div>div>div"),
    ).find((el) => {
    ...
    if (
      storyMenuBtn &&
      wrapperDiv.getElementsByClassName(CLASS_CUSTOM_BUTTON).length === 0
    ) {
      addCustomBtn(
        storyMenuBtn.parentElement?.parentElement?.parentElement?.parentElement?.parentElement as
          | Element
          | null,
        "white",
      );
  ...
  if (pathname.startsWith("/reels/")) {
    if (storageCache.settings.setting_enable_video_controls) {
      ...
      for (let i = 0; i < videos.length; i++) {
        if (videos[i].controls) continue;
        if (!videos[i].nextElementSibling?.hasAttribute("data-instancekey")) continue;
    ...
    for (const item of Array.from(reelsList)) {
  ...
  if (pathname.startsWith("/reel/")) {
    ...
    if (
      commentBtn &&
      (node as Document | HTMLElement).getElementsByClassName(CLASS_CUSTOM_BUTTON)
        .length === 0
    ) {
      addCustomBtn(
        commentBtn.parentElement?.parentElement?.parentElement?.parentElement?.parentElement as
          | Element
          | null,
        iconColor,
        "before",
  ...
  if (
    ...
    if (profileBtn) {
      addCustomBtn(
        profileBtn.parentElement?.parentElement?.parentElement as Element | null,
        iconColor,
      );
    }
  }

  // profile page video covers
  if (
...
function handleGlobalContextMenu(e: MouseEvent): void {
  ...
  if (btn.getAttribute("data-video-cover-download") === "true") return;
  e.preventDefault();
  onClickHandler(btn, true);
}
